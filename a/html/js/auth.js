...
function redirectByRole(auth, isRegister = false) {
  // Si es registro, ir al dashboard según rol
  if (isRegister) {
...
async function handleLogin(event) {
  ...
  try {
    ...
    saveAuth(auth);
    redirectByRole(auth, false); // isRegister = false
  } catch (error) {
...
async function handleRegister(event) {
  ...
  try {
    ...
    setTimeout(() => redirectByRole(auth, true), 400); // isRegister = true
  } catch (error) {
